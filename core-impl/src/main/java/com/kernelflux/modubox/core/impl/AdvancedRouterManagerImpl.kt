...
class AdvancedRouterManagerImpl : RouterManager {
    ...
    override fun unregisterRoute(path: String) {
        routes.remove(path)
        // 移除正则模式
        routePatterns.entries.removeIf { it.value.path == path }
    ...
    override fun fallback(originalPath: String, fallbackPath: String, params: Map<String, Any>?): Boolean {
    ...
    private class InterceptorChainImpl(
        ...
        override suspend fun proceed(path: String, params: Map<String, Any>?): Boolean {
            ...
            if (!interceptor.matches(path)) {
                return InterceptorChainImpl(index + 1, interceptors, routerManager).proceed(path, params)
            }
            
            return interceptor.intercept(path, params, InterceptorChainImpl(index + 1, interceptors, routerManager))
        }
    }
}
